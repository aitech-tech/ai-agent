...
:: ---- Read connector selection from recklabs_config.json (generated by recklabs.com) ----
set "INSTALL_CONFIG=%~dp0recklabs_config.json"
set "SELECTED_CONNECTORS=zoho"
if exist "%INSTALL_CONFIG%" (
    echo.
    echo Reading connector config: %INSTALL_CONFIG%
    for /f "delims=" %%s in ('powershell -NoProfile -Command "try { ((Get-Content '%INSTALL_CONFIG%' | ConvertFrom-Json).selected_connectors) -join ',' } catch { 'zoho' }"') do set "SELECTED_CONNECTORS=%%s"
    echo Selected connectors: !SELECTED_CONNECTORS!
) else (
    echo No connector config found — defaulting to: zoho
)

:: Write connector_config.json for the MCP server runtime
"%PYTHON_EXE%" -c "import json,sys; cfg={'selected_connectors':'!SELECTED_CONNECTORS!'.split(','),'generated_at':'%DATE%','version':'1.0.1'}; open(r'%AGENT_DIR%\config\connector_config.json','w').write(json.dumps(cfg,indent=2))"
