"""Payments Received — customer payments filtered by period."""
from products.zoho_books._base import (
    get_connector, cap_int, extract_records, group_amounts,
    filter_by_period, format_currency, totals_by_currency,
    success_response, error_response, _MULTI_CURRENCY_WARNING,
)

TOOL_NAME = "zb_payments_received"
TOOL_DESCRIPTION = (
    "Returns a pre-processed Payments Received summary for Zoho Books. "
    "Use for 'How much did I collect this month?', 'Show payment modes', 'Payments by customer'. "
...
_NAME_FIELDS = ["customer_name", "contact_name", "name"]
...
def run(params: dict) -> dict:
    ...
    try:
        result = connector.list_customer_payments(limit=limit)
    except Exception as e:
        return error_response("fetch_failed", str(e))

    all_records = extract_records(result, ["customer_payments"])
    ...
    if not filtered:
        return success_response(
            report="Payments Received", records_processed=0, records_returned=0,
            narrative_cue=f"No payments received found for period '{period}'.",
            period=period, date_range={"from": from_str, "to": to_str},
            payment_count=0, by_mode=[], top_customers=[],
    ...
    return success_response(
        report="Payments Received", records_processed=len(filtered), records_returned=len(top_customers),
        narrative_cue=(
            f"{len(filtered)} payments received for {period}"
            + (f" totalling {format_currency(single_total, single_code)}. " if not multi_currency else " across multiple currencies. ")
            + "Summarise by payment mode and top customers."
        ),
        period=period, date_range={"from": from_str, "to": to_str},
        payment_count=len(filtered), total_received=single_total,
        by_mode=by_mode, top_customers=top_customers,
